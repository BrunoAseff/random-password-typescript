...
function removeSimilarChars(characters: string): string {
  return characters
    .split("")
...
function generateRandomLength({
  ...
  maxLength,
  lengthRange,
}: Partial<PasswordOptions>): number {
  if (lengthRange) {
...
function validateOptions(options: PasswordOptions) {
  ...
  const conflicts = mustHave.some((item) =>
    Array.isArray(item)
  ...
  if (conflicts) {
    throw new Error(
      "Conflict between mustHave and excludeChars: can't both include and exclude the same characters."
    );
  }

  if (
    startsWith.split("").some((char) => excludeChars.includes(char)) ||
    endsWith.split("").some((char) => excludeChars.includes(char))
  ) {
    throw new Error(
      "Conflict between startsWith/endsWith and excludeChars: can't both include and exclude the same characters."
    );
  }

  if (lowercaseOnly && uppercaseOnly) {
    throw new Error(
      "Conflict: 'lowercaseOnly' and 'uppercaseOnly' cannot both be true."
    );
  }

  if (pattern && typeof length === "number" && length < pattern.length) {
    throw new Error(
      `Length of the password (${length}) is smaller than the pattern length (${pattern.length}).`
    );
  }
...
export function generatePassword(
  ...
  function createPassword(): string {
    ...
    const requiredLength =
      mustHave.join("").length + startsWith.length + endsWith.length;
    ...
    if (finalLength < requiredLength) {
      throw new Error(
        `The length provided (${finalLength}) is not sufficient to fit all required characters (mustHave, startsWith, endsWith). Minimum required length is ${requiredLength}.`
      );
    }
    ...
    if (
      excludeWords.some((word) =>
        passwordLowerCase.includes(word.toLowerCase())
      )
    ) {
